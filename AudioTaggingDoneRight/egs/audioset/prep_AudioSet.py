...
if __name__ == '__main__':
    ...
    list_path = "/jet/home/billyli/data_folder/AudioCLIP-/"
    vid_start_path = base_path+'/pylon5/ir3l68p/poyaoh/audioset'
    ...
    with open(base_path + '/data/datafiles/audioset_unbal_train_data_v' +'.json', 'w') as f:
    ...
    with open(os.path.join(list_path, 'balanced_train_segments.csv')) as bal_csv_file:
        ...
        for row in bal_csv_reader:
            if bal_count <3:
                ...
                vid_path = os.path.join(vid_start_path, 'featr_balanced', row[0]+'.npy')
                if os.path.exists(wav_path) and os.path.exists(vid_path):
                    cur_bal_dict = {"wav": wav_path, "labels": eval(','.join(row[3:])), "video": vid_path}
    #             print(cur_bal_dict)
    ...
    with open(base_path + '/data/datafiles/audioset_bal_train_data_v' +'.json', 'w') as f:
        json.dump({'data': bal_train_wav_list}, f, indent=1)

    with open(base_path + '/data/datafiles/audioset_bal_unbal_train_data_v' +'.json', 'w') as f:
    ...
    with open(os.path.join(list_path, 'eval_segments.csv')) as eval_csv_file:
        ...
        for row in eval_csv_reader:
            if eval_count <3:
                ...
                vid_path = os.path.join(vid_start_path, 'featr_eval', row[0]+'.npy')
                if os.path.exists(wav_path) and os.path.exists(vid_path):
                    cur_eval_dict = {"wav": wav_path, "labels": eval(','.join(row[3:])), "video": vid_path}
    #             print(cur_bal_dict)
    ...
    with open(base_path + '/data/datafiles/audioset_eval_data_v' +'.json', 'w') as f:
